...
int is_in_mandelbrot(int real_index, int img_index)
{
    ...
    c.real = ((double)real_index + parallel_drand()) / 1000;
    c.img  = ((double)img_index  + parallel_drand()) / 1000;
    z = c;
...
int main()
{
    ...
    for(int real_index = -2000; real_index < 500; real_index ++)
    {
        ...
        {
            int result = is_in_mandelbrot(real_index, img_index);
            #pragma omp atomic
            in_ctr += result;
            #pragma omp atomic
            total_ctr++;
